formsAngular
    .directive('formAngular', ['$compile', '$rootScope', function ($compile, $rootScope) {
        return {
            restrict: 'E',
            compile: function () {
...
// for angular 1.0 branch     '<div class="sub-doc well" id="' + info.id + 'List" ng-subdoc-repeat="subDoc in record.' + info.name + '">' +
                    ...
                    scope.$watch(attrs.schema, function (newValue) {

                        var elementHtml = '';

                        for (var aField = 0; aField < newValue.length; aField++) {
                            var info = newValue[aField];
                            if (info.directive) {
