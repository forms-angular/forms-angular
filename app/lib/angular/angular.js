            ...
            function ngModule($provide) {
                $provide.provider('$compile', $CompileProvider).
                    directive({
                        ...
                        ngFormRepeat: ngFormRepeatDirective,
 ...
 *                      'other': '{{person1}}, {{person2}} and {} other people are viewing.'}">
    ...
    var ngFormRepeatDirective = ngDirective({
        transclude: 'element',
        priority: 1000,
        terminal: true,
        compile: function (element, attr, linker) {
            return function (scope, iterStartElement, attr) {
                var expression = attr.ngFormRepeat;
                var match = expression.match(/^\s*(.+)\s+in\s+(.*)\s*$/),
                    lhs, rhs, valueIdent, keyIdent;
                if (!match) {
                    throw Error("Expected ngRepeat in form of '_item_ in _collection_' but got '" +
...
//                    arrayLength = array.length;
//
//                    for (index = 0; index < arrayLength && index < lastOrder.length; index++) {
//                        // we are not using forEach for perf reasons (trying to avoid #call)
//                        key = (collection === array) ? index : array[index];
//                        value = collection[key];
//
//                        last = lastOrder[index];
//                        childScope = last.scope;
//                        nextOrder.push(value, last);
//                        cursor = last.element;
//
...
//
//                    //shrink children
//                    for (index = arrayLength; index < lastOrder.length; index++) {
//                        array = lastOrder[index];
//                        value = array.pop();
//                        value.element.remove();
//                        value.scope.$destroy();
//                    }
