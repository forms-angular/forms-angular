...
const ExamSchemaDef: IFngSchemaDefinition = {
  subject: String,
...
const ExamsSchema = new Schema(ExamSchemaDef,  {_id: false});
...
const GSchema = new Schema(GSchemaDef);

let G;
try {
  G = model('g_conditional_field');
} catch (e) {
  G = model('g_conditional_field', GSchema);
}
