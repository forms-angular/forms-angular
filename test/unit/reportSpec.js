    ...
    describe('url handling', function() {
        ...
        it('should support pipelines in report instructions in query', function() {
            inject(function (_$httpBackend_, $rootScope, $controller, $location, $routeParams) {
                ...
                $httpBackend.whenGET('/api/collection?p=[{"$project":{"name":1}}]').respond([{"_id":"F","count":11},{"_id":"M","count":6}]);

                $location.$$path = '/collection/analyse';
                $routeParams.r = '{"pipeline":[{"$project":{"name":1}}]}'
                scope = $rootScope.$new();
        ...
        it('should support report instructions which are fetched from server', function() {
            inject(function (_$httpBackend_, $rootScope, $controller, $location, $routeParams) {
                $httpBackend = _$httpBackend_;
                $httpBackend.whenGET('/api/report-schema/collection/myReport').respond({"pipeline":[{"$project":{"name":1}}]});
                $httpBackend.whenGET('/api/collection?p=[{"$project":{"name":1}}]').respond([{"_id":"F","count":11},{"_id":"M","count":6}]);

                $location.$$path = '/collection/analyse';
                $routeParams.r = 'myReport'
                scope = $rootScope.$new();
                ctrl = $controller("AnalysisCtrl", {$scope: scope});
                $httpBackend.flush();
                expect(scope.report.length).toBe(2);
