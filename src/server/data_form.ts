    ...
    for (let element in paths) {
        if (paths.hasOwnProperty(element) && element !== '__v') {
            ...
            if (paths[element].schema) {
                ...
                // ...to also support individual properties of nested schemas wishing to use
                // the .secure attribute, we would need to do something with
                // subSchemaInfo.hide here
            } else {
                // check for arrays
...
DataForm.prototype.filteredFind = function (
    resource: Resource,
    req: Express.Request,
