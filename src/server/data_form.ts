...
export class FormsAngular {
        ...
        if (schema.hasOwnProperty(fld)) {
          if (vanilla[fld]) {
            ...
            if (vanilla[fld].schema) {
              outPath[fld].schema = this.applySchemaSubset(
                outPath[fld].schema,
                schema[fld].schema
              );
            }
      ...
      for (let element in paths) {
        if (paths.hasOwnProperty(element) && element !== "__v") {
          ...
          if (paths[element].schema) {
            let subSchemaInfo = this.preprocess(
              null,
              paths[element].schema.paths
            );
  ...
  schema() {
    return _.bind(function (req, res) {
      ...
      if (req.resource.preprocessed?.[formName || "__default"]) {
        ...
        if (formName) {
          try {
            formSchema = req.resource.model.schema.statics["form"](
              escapeHtml(formName),
              req
            );
          } catch (e) {
        ...
        let paths = this.preprocess(
          req.resource,
          req.resource.model.schema.paths,
          formName,
          formSchema
        ).paths;
        res.send(paths);
  ...
  report() {
    return _.bind(async function (
      ...
      if (req.params.reportName) {
        try {
          reportSchema = await req.resource.model.schema.statics["report"](
        ...
        case "$graphLookup":
          ...
          if (resource) {
            ...
            if (resource.options?.hide?.length > 0) {
              const hiddenLookupFields = this.generateHiddenFields(
                resource,
                false
              );
              let hiddenFieldsObj = {};
            ...
            if (needFindFunc && resource.options.findFunc) {
              ...
              if (!nextStageIsUnwind) {
                throw new Error(
                  "No support for $lookup where the next stage is not an $unwind and the resources has a findFunc"
                );
              }
              // Push the $unwind, add our own findFunc, and increment the pipelineStage counter
              retVal.push(array[pipelineSection + 1]);
              let lookedUpFindQry: FilterQuery<any> =
                await this.doFindFuncPromise(req, resource);
              if (lookedUpFindQry) {
  ...
  reportInternal(req, resource, schema, callback) {
    ...
    self.doFindFunc(req, resource, function (err, queryObj) {
      if (err) {
            ...
            if (req.query[param]) {
              if (param !== "r") {
                ...
                if (schema.params[param] !== undefined) {
                  ...
                  return callback(
                    `No such parameter as ${param} - try one of ${Object.keys(schema.params).join()}`
                  );
                }
              }
            }
        ...
        if (runPipelineStr) {
          runPipelineStr = runPipelineStr.replace(
            /"\(.+?\)"/g,
            function (match) {
              ...
              if (sparam !== undefined) {
                ...
                return callback(
                  `No such parameter as ${match.slice(2, -2)} - try one of ${Object.keys(schema.params).join()}`
                );
              }
            }
          );
        ...
        if (schema.columnTranslations) {
              ...
              function doATranslate(column, theTranslation) {
                results["runAggregation"].forEach(function (resultRow) {
                  ...
                  valToTranslate = valToTranslate
                    ...
                    : "";
                  let thisTranslation = _.find(
                    theTranslation.translations,
                    function (option) {
          ...
          for (let i = 0; i < schema.columnTranslations.length; i++) {
            ...
            if (thisColumnTranslation.field) {
              ...
              if (
                !thisColumnTranslation.translations &&
                !thisColumnTranslation.ref &&
      ...
      doc
        ...
        .then((saved) => {
          ...
          for (const hiddenField in hiddenFields) {
            if (
              hiddenFields.hasOwnProperty(hiddenField) &&
  ...
  async filteredFind(
    ...
    async function doAggregation(queryObj, cb) {
      if (aggregationParam) {
        aggregationParam = await that.sanitisePipeline(
          aggregationParam,
          hiddenFields,
          queryObj,
          req
        ...
        resource.model
          ...
          .then((aggregationResults) => {
            ...
            cb(
              _.map(aggregationResults, function (obj) {
                return obj._id;
              })
            );
          })
      ...
      if (err) {
        ...
        doAggregation(queryObj, function (idArray) {
          if (aggregationParam && idArray.length === 0) {
            ...
            query = query.select(
              that.generateProjection(hiddenFields, projectParam)
            );
            if (limit) {
              ...
              .then((docs) => {
                if (stashAggregationResults) {
                  for (const obj of docs) {
                    ...
                    let aggObj = stashAggregationResults.find(
                      (a) => a._id.toString() === obj._id.toString()
                    );
                    for (const k of Object.keys(aggObj).filter((k) =>
                      k.startsWith("__")
                    )) {
                      obj[k] = aggObj[k];
                    }
      ...
      this.processEntity(req, res, function () {
        ...
        for (let prop in cleansedBody) {
          if (cleansedBody.hasOwnProperty(prop)) {
            req.doc.set(
              prop,
              cleansedBody[prop] === "" ? undefined : cleansedBody[prop]
            );
          }
        }
  ...
  generateDependencyList(resource: Resource) {
    if (resource.options.dependents === undefined) {
      ...
      for (let pluginName in that.options.plugins) {
        ...
        if (
          thisPlugin.dependencyChecks &&
          thisPlugin.dependencyChecks[resource.resourceName]
        ) {
          resource.options.dependents = resource.options.dependents.concat(
            thisPlugin.dependencyChecks[resource.resourceName]
          );
        }
      }
    ...
    return _.bind(async function (
      ...
      async function removeDoc(
        ...
        switch (resource.options.handleRemove) {
          ...
          default:
            return that
              .getDependencies(resource, doc._id as Types.ObjectId)
              .then((dependencies) => {
  ...
  buildSingleResourceAmbiguousRecordStore<t>(
    items: t[],
    equalityProps: string[],
    disambiguationResourceName: string
  ...
  buildMultiResourceAmbiguousRecordStore<
    t extends { [prop in f]?: string },
    f extends string,
  >(
    items: t[],
    equalityProps: string[],
    ...
    for (let i = 0; i < items.length - 1; i++) {
      for (let j = i + 1; j < items.length; j++) {
        if (
          ...
          if (
            !store[items[i][disambiguationResourceNameProp]].includes(items[i])
          ) {
            store[items[i][disambiguationResourceNameProp]].push(items[i]);
          }
          if (
            !store[items[i][disambiguationResourceNameProp]].includes(items[j])
          ) {
    ...
    this.filteredFind(
      ...
      function (err, docs) {
        if (err) {
          ...
          if (concatenateParam) {
            const transformed: fngServer.DisambiguatableLookupItem[] = docs.map(
              (doc: any) => {
                let text = "";
                for (const field of listFields) {
                  if (doc[field]) {
            ...
            if (disambiguationResourceName) {
              that.disambiguate(
                transformed,
                that.buildSingleResourceAmbiguousRecordStore(
                  transformed,
                  ["text"],
                  disambiguationResourceName
                ),
                "disambiguationId",
                (
                  item: fngServer.DisambiguatableLookupItem,
